...
function addEventListeners() {
	...
	buttonArray = [
		contentContainer, closeAdButton,
...
function createTimeline() {
	...
	tlAnim 	.addLabel("start")
			...
			.to([logo2, text4], {duration:0.5, autoAlpha:0, ease:"quad.inOut", overwrite:false}, "frame5")
			.to(text4, {duration:1, y:"-=400", ease:"quad.inOut", overwrite:false}, "frame5")
			
			.fromTo(text5, {y:"+=200"}, {duration:0.75, y:0, autoAlpha:1, ease:"quad.inOut", overwrite:false}, "frame5+=0.5")
			.fromTo(text6, {y:"+=200"}, {duration:0.75, y:0, autoAlpha:1, ease:"quad.inOut", overwrite:false}, "frame5+=0.55")
			.fromTo(dynamicTerms, {y:"+=200"}, {duration:0.75, y:0, autoAlpha:1, ease:"quad.inOut", overwrite:false}, "frame5+=0.6")
...
function showCTA($xPos) {
	...
	gsap.from(split.words, {duration:0.5, x:$xPos, ease:"quad.inOut", force3D:false, rotationZ:0.01, stagger:0.1, overwrite:false, onComplete:function() {
		animateCTA();
	}});
}
	...
	switch (button) {
		...
		case termsContainer:
			if 		(type === "mouseover") 	{  }
												...
												$e.stopPropagation();
												gsap.to(termsContainer, {duration:0.5, top:480, ease:"power2.inOut", onComplete:function() {
